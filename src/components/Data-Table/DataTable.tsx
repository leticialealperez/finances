...
import { MyTableHead } from './Table-Header/TableHeader';
import { MyTableToolbar } from './Table-Toolbar/TableToolbar';
...
const DataTable: React.FC<TableProps> = ({ title, data }) => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof IData>('value');
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <MyTableToolbar numSelected={selected.length} title={title}/>
        <TableContainer>
          <Table
            sx={{ minWidth: 550 }}
